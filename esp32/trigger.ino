...
void showAngry() {
  ...
  // V-shape: left brow inner edge down, right mirror
...
void loop() {
  ...
  while (Serial2.available()) {
    ...
    Serial.print("[ESP32] Raw byte received: ");
    Serial.print(c);
    Serial.print(" (");
    if (c >= 32 && c < 127) Serial.print((char)c);
    else if (c == '\r') Serial.print("CR");
    else if (c == '\n') Serial.print("LF");
    else Serial.print("0x"), Serial.print(c, HEX);
    Serial.println(")");

    // Immediate raw numeric code (0,1,2,3)
    if (c >= 0 && c <= 3) {
      Serial.print("[ESP32] Received raw code: ");
      Serial.println(c);
      if (c != lastExpressionCode) {
        lastExpressionCode = c;
        if (c == 0) showAngry();
        else if (c == 1) showHappy();
        else if (c == 2) showSad();
        else if (c == 3) showNeutral();
      } else {
        Serial.println("[ESP32] Same expression, skipping animation");
      }
      incomingLine = "";
      continue;
    }

    // ASCII digits form a line
    if (c == '\r' || c == '\n') {
      if (incomingLine.length() > 0) {
        Serial.print("[ESP32] Complete line: ");
        Serial.println(incomingLine);

        int classCode = incomingLine.toInt();
        incomingLine = "";

        if (classCode >= 0 && classCode <= 3) {
          if (classCode == lastExpressionCode) {
            Serial.println("[ESP32] Same expression, skipping animation");
          } else {
            lastExpressionCode = classCode;
            Serial.print("[ESP32] Received class code: ");
            Serial.println(classCode);
            if (classCode == 0) showAngry();
            else if (classCode == 1) showHappy();
            else if (classCode == 2) showSad();
            else if (classCode == 3) showNeutral();
          }
        } else {
          Serial.print("[ESP32] Invalid code line: ");
          Serial.println(classCode);
        }
      }
    } else if (c >= '0' && c <= '3') {
      ...
      Serial.print("[ESP32] Skipping invalid byte: ");
      Serial.println(c);
    }
  }

  // Idle blink every ~5 seconds
  static unsigned long lastBlink = 0;
  if (millis() - lastBlink > 5000) {
    lastBlink = millis();
    if (lastExpressionCode == 1)
      blinkEyes(drawCrescentEye);   // happy
    else if (lastExpressionCode == 0)
      blinkEyes(drawSquintEye);     // angry
    else if (lastExpressionCode == 2)
      blinkEyes(drawSadEye);        // sad
    else
      blinkEyes(drawNeutralEye);    // neutral or unset
  }
}
